$(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 200) {
                    ...
                    setTimeout(function () {
                        toastr['success'](
                            Lang.get('js_local.User_Deleted_Successfully'),
                            {
                                closeButton: true,
                                tapToDismiss: false,
                                rtl: isRtl
    ...
    $('#status-sub').on('show.bs.modal', function (e) {
        ...
        $('#user_id').val(course_id);



        if (status == 1) {
            $('#status_btn').text(Lang.get('js_local.Block'));
            $('#itemDeleteModalTitle').text(Lang.get('js_local.user_block_warning'));
            $('#myModalLabel22').text(Lang.get('js_local.Block'));
        } else {
            $('#status_btn').text(Lang.get('js_local.Activate'));
            $('#itemDeleteModalTitle').text(Lang.get('js_local.user_activate_warning'));
