$(function () {
    ...
    $("#delete_btn").on('click', function (e) {
        ...
        $.ajax({
            ...
            url: '/admin/teacher/delete/' + sub_id,
            ...
            success: function (response) {
                if (response.status === 200) {
                    ...
                    setTimeout(function () {
                        toastr['success'](
                            Lang.get('js_local.Teacher_Deleted_Successfully'),
                            {
                                closeButton: true,
                                tapToDismiss: false,
                                rtl: isRtl
    ...
    $('#status-sub').on('show.bs.modal', function (e) {
        ...
        $('#teacher_id').val(course_id);



        if (status == 1) {
            $('#status_btn').text(Lang.get('js_local.Block'));
            $('#itemDeleteModalTitle').text(Lang.get('js_local.teacher_block_warning'));
            $('#myModalLabel22').text(Lang.get('js_local.Block'));
        } else {
            $('#status_btn').text(Lang.get('js_local.Activate'));
            $('#itemDeleteModalTitle').text(Lang.get('js_local.teacher_activate_warning'));
