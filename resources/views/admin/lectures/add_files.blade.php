...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"> {{trans('lectures/lectures.Lecture')}} </h4><span
                    class="text-muted mt-1 tx-13 ms-2 mb-0">/  {{trans('lectures/lectures.Add Files')}}</span>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="pull-right">
                        <a class="btn btn-primary btn-sm" href="{{ route('lecture.index') }}"> {{trans('lectures/lectures.Back')}}</a>
                    </div>
                        <form action="{{route('lecture.Attachments')}}" method="POST"
                          id="lecture_form" data-parsley-validate="">
                        @csrf
                        <div class="tab-pane " id="files">
                            <div class="card-body">
                                <p class="text-danger">* Form: pdf,jpeg ,jpg ,png,zip </p>
                                <h5 class="card-title"> {{trans('lectures/lectures.Add Attachments')}}</h5>
                                <form method="" action=""
                                      enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label for="RePassword"> {{trans('lectures/lectures.name')}}</label>
                            ...
                            <div class="table-responsive mt-15">
                                <table class="table center-aligned-table mb-0 table table-hover"
                                    ...
                                    <tr class="text-dark">
                                        <th scope="col">#</th>
                                        <th scope="col"> {{trans('lectures/lectures.file name')}}</th>
                                        <th scope="col"> {{trans('lectures/lectures.created at')}}</th>
                                        <th scope="col"> {{trans('lectures/lectures.methods')}}</th>
                                    </tr>
                                    </thead>
                                    <tbody id="list">
                                    <?php $i = 0; ?>
                                        ...
                                        <tr id="row-{{$attachment->id}}">
                                            ...
                                            <td colspan="2">
                                                <a class="btn btn-success btn-sm"
                                                   href="{{ route('lecture.View_file',['path'=>$attachment->path,'file_name'=>$attachment->name])  }}"
                                                   role="button"><i class="fas fa-eye"></i>&nbsp;
                                                </a>

                                                <a class="btn btn-info btn-sm"
                                                   href="{{ route('lecture.download',['path'=>$attachment->path,'file_name'=>$attachment->name])  }}"
                                                   role="button"><i
