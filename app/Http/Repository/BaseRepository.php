...
//    }
    ...
    public function paginate($per_page_num, $conditions = [], $columns = array('*'))
    {
        ...
        try {
            ...
            $order_by_direction = isset($this->requestData[Constants::ORDER_By_DIRECTION]) ? $this->requestData[Constants::ORDER_By_DIRECTION] : "asc";
            $filter_operator = isset($this->requestData[Constants::FILTER_OPERATOR]) ? $this->requestData[Constants::FILTER_OPERATOR] : "=";
            $filters = isset($this->requestData[Constants::FILTERS]) ? $this->requestData[Constants::FILTERS] : [];
//            dd($filters);

            $per_page = isset($this->requestData[Constants::PER_PAGE]) ? $this->requestData[Constants::PER_PAGE] : $per_page_num;
            $query = $this->model;
            $allConditions = array_merge($conditions, $filters);
            if (isset($order_by))
